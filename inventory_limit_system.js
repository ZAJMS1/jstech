...
try {
    ...
    // Also update the updateUI function to show item count
    const updateUIIndex = content.indexOf('updateUI: function()', inventorySystemIndex);
    if (updateUIIndex !== -1) {
        // Find where the HTML is generated
        const updateHtmlIndex = content.indexOf('let itemsHtml', updateUIIndex);
        if (updateHtmlIndex !== -1) {
            // Find the beginning of the line that creates the title
            const titleLineIndex = content.indexOf('itemsHtml +=', updateHtmlIndex);
            if (titleLineIndex !== -1) {
                // Original line
                const originalTitleLine = content.substring(titleLineIndex, content.indexOf(';', titleLineIndex) + 1);
                // New line with item count
                const newTitleLine = `itemsHtml += '<h3 style="margin-top:0">Inventory (' + this.items.filter(item => !item.used).length + '/3)</h3>';`;
                // Replace
                content = content.replace(originalTitleLine, newTitleLine);
            }
        }
    }
    
    // Fix the title line to use a function instead of a hardcoded count
    content = content.replace(`itemsHtml += '<h3 style="margin-top:0">Inventory (${this.items.filter(item => !item.used).length}/3)</h3>';`, 
