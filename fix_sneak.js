...
try {
    ...
    // Set hasBackpack to true for the sneaking route
    const backpackPattern = /(const gameProgress = \{[\s\S]*?)hasBackpack:.*?,/g;
    content = content.replace(backpackPattern, '$1hasBackpack: true, // Player has backpack for sneaking route');
    
    // Add a custom message about sneaking path
    const messagePattern = /\/\/ Position player/;
    content = content.replace(messagePattern, '// Display sneaking path message\n    displayMessage("You\'ve chosen to sneak past the checkpoint. Stay hidden in the shadows.", 5000);\n\n    // Position player');
    
    // Write the modified content back to the file
    fs.writeFileSync(filePath, content);
    console.log('Successfully removed cutscene functionality from sneak.html while maintaining player control!');
    
} catch (error) {
    console.error('Error modifying sneak.html:', error);
} 