...
// Read the alternative.html file
try {
    ...
    // Set hasBackpack to true for the alternative route
    const backpackPattern = /(const gameProgress = \{[\s\S]*?)hasBackpack:.*?,/g;
    content = content.replace(backpackPattern, '$1hasBackpack: true, // Player has backpack for alternative route');
    
    // Add a custom message about alternative path
    const messagePattern = /\/\/ Position player/;
    content = content.replace(messagePattern, '// Display alternative path message\n    displayMessage("You\'ve chosen the alternative route. Find the underground passage.", 5000);\n\n    // Position player');
    
    // Write the modified content back to the file
    fs.writeFileSync(filePath, content);
    console.log('Successfully removed cutscene functionality from alternative.html while maintaining player control!');
    
} catch (error) {
    console.error('Error modifying alternative.html:', error);
} 